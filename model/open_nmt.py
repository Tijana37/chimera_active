...
class OpenNMTModel(Model):
    ...
    def run_traslate(self, input_file, output_file, opt):
        print("model bin path:", self.model_bin_path)
        if  hasattr(self, "model_bin_path") or self.model_bin_path is None:   # TODO remove after EMNLP
            print("START")
            self.model_bin_path = save_temp_bin(self.model_bin)

            print("END", self.model_bin_path)
